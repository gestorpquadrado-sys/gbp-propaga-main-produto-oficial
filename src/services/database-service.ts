...
export async function getDirectoryStatusByGoogleLocationId(googleLocationId: string) {
  ...
  const results = Object.fromEntries((data || []).map((row: any) => [
    row.channel_code,
    {
      ...
      submissionType: row.submission_type || row.raw_response?.submissionType || undefined,
      directoryName: row.directory_name || row.raw_response?.directoryName || undefined,
      formUrl: row.form_url || row.raw_response?.formUrl || undefined,
      websiteUrl: row.raw_response?.websiteUrl || undefined,
      profileUrl: row.profile_url || row.raw_response?.profileUrl || undefined,
      verificationStatus: row.verification_status || row.raw_response?.verificationStatus || undefined,
      verificationMessage: row.verification_message || row.raw_response?.verificationMessage || undefined,
      nextVerificationAt: row.next_verification_at || row.raw_response?.nextVerificationAt || undefined,
      manualInstructions: row.raw_response?.manualInstructions || undefined,
      copyPayload: row.raw_response?.copyPayload || undefined,
    },
  ]));
